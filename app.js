...
// Word Wand v9.3.9.2 — legibility: 10 rows, thinner strokes, multi-tap sampling, lighter afterglow
window.addEventListener('error', e => console.log('WW v9.3.9.2 runtime error:', e.message));
...
// === Text → columns with spacing; higher vertical resolution ===
const NUM_ROWS=10;               // ↑ from 8 → 10
const VERTICAL_DILATE=1;         // ↓ thinner strokes
const LUMA_THR=90;               // ↑ threshold → thinner
const PAD_BETWEEN_CHARS=4, PAD_BETWEEN_WORDS=10;
let textColumns=[], colCount=0;

function sampleGlyphColumns(char){
  const fs=320; // slightly larger sampling size
  ...
  const m=g.measureText(char); const W=Math.max(1, Math.ceil(m.width + fs*0.16)), H=Math.ceil(fs*1.5);
  ...
  const img=g.getImageData(0,0,W,H).data; const cols=[];
  for(let x=0;x<W;x++){
    const col=new Uint8Array(NUM_ROWS);
    for(let r=0;r<NUM_ROWS;r++){
      // multi-tap sampling across sub-rows to preserve diagonals
      const taps=3; let on=0;
      for(let t=0;t<taps;t++){
        const fr=(r + (t/(taps-1)) - 0.5)/NUM_ROWS; const y=Math.max(0, Math.min(H-1, Math.floor(fr*H)));
        const idx=(y*W+x)*4; const lum=img[idx]*0.2126+img[idx+1]*0.7152+img[idx+2]*0.0722; if(lum>LUMA_THR){ on=1; break; }
      }
      col[r]=on;
    }
...
function buildColumns(){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    const gap=(i<raw.length-1 && raw[i+1]!==' ')?PAD_BETWEEN_CHARS:0; for(let p=0;p<gap;p++) cols.push(new Uint8Array(NUM_ROWS));
  }
...
function updateMotion(){
  filtYaw=0.86*filtYaw + 0.14*currentYaw; const vel=filtYaw - prevFiltYaw; prevFiltYaw=filtYaw; filtVel=0.82*filtVel + 0.18*vel; const speeding=Math.abs(filtVel)>VEL_THR; sweepBoost=sweepBoost*0.85 + (speeding?1:0)*0.15; const now=performance.now(); if(speeding && (now-lastFlipTs)>FLIP_COOLDOWN){ const nd=(filtVel>0)?'ltr':'rtl'; if(nd!==lastDir){ lastDir=nd; lastFlipTs=now; } }
}

// Draw POV (afterglow slightly reduced)
...
function drawPOV(){
  ...
  // Lighter afterglow to avoid solid bars
  ctx.fillStyle='rgba(0,0,0,0.05)'; ctx.fillRect(0,0,w,h);
  const base=0.96, heightFactor= base + (1.0-base)*sweepBoost; const spacing=(h*heightFactor)/NUM_ROWS; const diam=Math.min(spacing, Math.max(7, spacing*0.84)); const gap=Math.max(1, spacing-diam);

...
function scaleRow(sr){ return Math.max(0, Math.min(6, Math.round(sr*(7-1)/(NUM_ROWS-1)))); }
function buildColumns(){
  ...
  for(let i=0;i<raw.length;i++){
    ...
    for(let c=0;c<glyph.length;c++){
      const bits=glyph[c]; const col=new Uint8Array(NUM_ROWS);
      for(let r=0;r<NUM_ROWS;r++){ const fr=scaleRow(r); col[r]=((bits>>fr)&1)?1:0; }
      cols.push(col);
    }
